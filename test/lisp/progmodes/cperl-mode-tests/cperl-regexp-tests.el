;;; sane-perl-regexp-tests.el --- Test basic regular expressions in sane-perl-mode -*- lexical-binding: t -*-
...
(ert-deftest sane-perl-test-basic-identifier-regexp ()
  "Verify that basic identifiers are found.
This is sort of a unit test for some of the regular expressions
in sane-perl-mode."
  (should (equal (string-match sane-perl--basic-identifier-regexp
                               "3 + $foo+$bar")
                 5))    ;; plain scalar
  ...
  (should (equal (string-match sane-perl--identifier-regexp
                               "3 + $Foo::bar*3")
                 5))    ;; an identifier with package spec
  ...
  (should (equal (string-match sane-perl--version-regexp
                               " v1.2.3 ")
                 1))    ;; "structured" version

  (should (equal (string-match (concat "^\\("
                                       sane-perl--version-regexp
                                       "\\)$")
                               " v1.2 ")
...
(defun sane-perl-test-wrap-podlink-process (text expected)
  "Process TEXT with `sane-perl--pod-process-links', check against EXPECTED.
This is a helper for `sane-perl-test-podlink-process' to reduce
