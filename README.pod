...
examples. Generally we are using the examples, but K&R style, for
example, is given as
...
in the cperl-mode documentation, which is definitely wrong, see L<this
Wikipedia
article|https://en.wikipedia.org/wiki/Indentation_style#K&R_style>.
In fact cperl mode actually produces

     if (foo) {

anyway. 

The styles affected are C<BSD>, C<K&R>, C<GNU>, and C<Whitesmith>.
C<C++>, C<PBP>, and C<PerlStyle>, and the default C<Sane-Perl> were
already indenting as per the documentation. At the time of writing the
C<Whitesmith> style is the only one where the documentation and
results disagree, on only one line.
...
A test of fix of the semicolon bug, where C<};> (for example at the
end of a BEGIN block) used to be mistakenly converted into
