...
The styles where the example did not match the output are C<BSD>,
C<K&R>, C<GNU>, and C<Whitesmith>.  C<C++>, C<PBP>, and C<PerlStyle>,
and the default C<Sane-Perl> were already indenting as per the
documentation. We have altered either the settings or the example to
bring them into agreement, with the exception of the C<Whitesmith>
