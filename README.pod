...
in the documentation, which is definitely wrong, see L<this Wikipedia
article|https://en.wikipedia.org/wiki/Indentation_style#K&R_style> for
...
We are trying to sort this out, but our assumptions may be wrong. The
styles affected are the C<BSD>, C<K&R>, C<GNU>, and C<Whitesmith>
styles. The C<C++>, C<PBP>, and C<PerlStyle>, and the default
C<Sane-Perl> styles were already indenting as per the
documentation. At the time of writing the C<Whitesmith> style is the
only one where the documentation and results disagree, on only one
line.
...
nested Pod commands within links. See L</Pod parsing>. These have been
fixed in sane-perl-mode.el. Tests are in F<t/nested-pod.t>.
