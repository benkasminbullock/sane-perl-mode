...
we are not sure whether to modify the indentation styles so that they
produce the examples, or modify the examples to produce the same thing
as the modes. For the most part we have never used these styles, so we
are not sure what should actually be produced. Generally we are using
the examples, but some of the examples are quite obviously wrong. K&R
style, for example is given as
...
in the documentation, which is definitely wrong, see
L<https://en.wikipedia.org/wiki/Indentation_style#K&R_style> for
example, but cperl-mode actually produces

     if (foo) {

anyway. 

So while we are doing our best to sort this out, our assumptions may
be wrong, and this may need to be changed. The styles affected are the
C<BSD>, C<K&R>, C<GNU>, and C<Whitesmith> styles. The C<C++>, C<PBP>,
and C<PerlStyle>, and the default C<Sane-Perl> styles were already
indenting as per the documentation. At the time of writing the
Whitesmith style is the only one where the documentation and results
disagree.

=head2 Pod parsing

Some bugs parsing Pod links which would be seen on using
C<sane-perl-perldoc> have been fixed.
...
turned off, even though font-lock-mode is on by default. We have
discovered that the old default of having C<sane-perl-font-lock>
switched off with font-lock-mode on causes some odd behaviour, so
we've switched it on as the default. It can still be switched off
using

    (setq sane-perl-font-lock nil)

In this case, font-lock-mode itself will be switched off as well,
otherwise the option doesn't really make sense.
...
Cperl mode parses pod before displaying it in a help buffer to change
the links to an Emacs-valid format. There were some problems with
parsing nested Pod commands within links, which have been fixed in
sane-perl-mode.el. Tests are in F<t/nested-pod.t>.
...
A test of the semicolon bug where C<};> is converted into

    }
    ;

on applying C<indent-region> is in F<t/semicolon-bug.t>. The bug is
not fixed yet.
...
to C<-sbl> in perltidy.

=item sane-perl-merge-trailing-else

This option is C<t> by default. Setting this to C<t> is equivalent to
C<-ce> in perltidy.
